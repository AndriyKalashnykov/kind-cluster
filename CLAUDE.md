...
- **cloud-provider-kind is started with `--gateway-channel disabled`** (`scripts/kind-add-cloud-provider-kind.sh`). The lab serves the Gateway API via dedicated controllers (Traefik/Istio/NGF/Contour/Envoy/kgateway/Kong), never via CPK's own Gateway API support, and CPK assigns LoadBalancer IPs to Services independently of its gateway channel (`controller.go:238-253` runs before any channel check @ v0.10.0). By **default** (`--gateway-channel standard`) CPK force-installs its embedded Gateway API CRDs (v1.4.0) at startup; on a cluster that already has the experimental Gateway API v1.5.1 CRDs + their `safe-upgrades` ValidatingAdmissionPolicy (which denies installing any GW API CRD < v1.5.0), that install is **denied** and CPK aborts (`Failed to start cloud controller`) — it then never assigns a LoadBalancer IP. This bit the **MetalLB → CPK migration** whenever a gateway controller was already installed (the migration starts CPK *after* those CRDs + VAP exist). `disabled` skips CPK's GW API CRD reconcile entirely (verified by source + an A/B spike: no-flag → crash + no IP; `disabled` → clean start + IP). Harmless everywhere else — nothing in the lab relies on CPK installing GW API CRDs (`scripts/kind-add-gateway-api-crds.sh` owns that, installing the experimental v1.5.1 channel).
...
- [ ] **cloud-provider-kind is pre-1.0** (0.10.0) and load-bearing for `make install-all` — watch its changelog for breaking changes on each minor bump. In particular, the `--gateway-channel disabled` flag in `scripts/kind-add-cloud-provider-kind.sh` is now **load-bearing** (it stops CPK's embedded pre-v1.5.0 Gateway API CRD reconcile from tripping the experimental-channel `safe-upgrades` VAP during the MetalLB→CPK migration); re-verify the flag still exists and behaves on every CPK bump (`docker run …:vX.Y.Z --help`). The `gateway-test.yml` "Regression guard — CPK survives a restart …" step fails if the flag is dropped.
- [x] **DONE (2026-06-24): in-cluster NFS migrated off the mutable-tag `itsthenetwork/nfs-server-alpine:12` to `sharedvolume/nfs-server:v0.1.0` (immutable semver tag, digest-pinned).** Root cause of the recurrence (#149, #154): the `kubernetes` Renovate manager digest-pins `k8s/nfs/nfs-server.yaml`, but `:12` is a **mutable** tag that upstream periodically re-pushes — re-resolving it orphans the old digest into an unpullable config-blob (`docker manifest inspect …@sha256:79f203e…` → `manifest schema unsupported`), so Renovate's digest-bump PR breaks e2e at *Install in-cluster NFS* (the `nfs-server` Deployment can't pull → never rolls out). The real fix is **an immutable tag**, not disabling digest tracking: `sharedvolume/nfs-server` is a verbatim 2025 fork of itsthenetwork (identical `/usr/bin/nfsd.sh` entrypoint, same `SHARED_DIRECTORY` + `fsid=0` export-as-`/` contract → drop-in for `share: /`, NFSv4.1, multi-arch amd64+arm64) pinned on semver `v0.1.0`, so digest re-resolution is stable and a future `v0.2.0` is a real reviewable bump, not a silent orphaning. Validated by a local kind spike running the real `scripts/kind-add-nfs-incluster.sh`: nfs-server 1/1, `demo-claim-incluster` RWX PVC **bound**, plus cross-pod RWX read/write (writer wrote, separate reader read). **Residual (watch, low-stakes):** `sharedvolume/nfs-server` is a v0.x single-author project (~4★, first build Aug 2025) — low bus-factor, but the digest pin means even if it goes dark the image keeps working (immutable tag → no orphaning). If it's ever yanked or a CVE needs patching with no successor release, the documented fallback is to re-pin itsthenetwork at its then-current resolved digest and close future mutable-tag bumps (the pre-migration behavior). Upstream csi-driver-nfs itself still references `itsthenetwork/nfs-server-alpine:latest`, so there is no "blessed" alternative to track.
