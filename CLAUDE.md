...
- [ ] **`CLOUD_PROVIDER_KIND_VERSION` tracks the published CONTAINER IMAGE, not the GitHub release (`datasource=docker depName=registry.k8s.io/cloud-provider-kind/cloud-controller-manager`).** The value is consumed only as a `docker run …:v$(VERSION)` tag, and CPK's image reaches `registry.k8s.io` via a manual kpromo promotion PR to `kubernetes/k8s.io` that lags the GitHub release by hours-to-days. A `github-releases` datasource (the prior pin) proposed v0.11.0 the day it was tagged (2026-06-24) while the image was unpublished, breaking e2e at `docker run` (`manifest unknown`) before any LB IP — Renovate PR #152. Do NOT switch this back to `github-releases`. Renovate will propose the next bump only once the matching image tag is promoted. (Same datasource-vs-sink rule as the `/renovate` skill's "track the CONSUMED SINK" section.)
