...
- [ ] **`itsthenetwork/nfs-server-alpine:12` is an unmaintained, MUTABLE-tag third-party image — evaluate a maintained replacement.** The `:12` tag is occasionally re-pushed upstream, which orphans the previously-pinned digest into an unpullable *blob* (`docker pull …@sha256:<old>` → `unexpected media type application/octet-stream … not found`). When that happens Renovate opens a spurious digest-bump PR to the orphaned digest (e.g. #149/#154, `…@sha256:79f203e…`) that fails the e2e `Install in-cluster NFS` step (the `nfs-server` Deployment never becomes ready). The CI gate catches it; close such PRs — the current pin (what `:12` resolves to today) is correct. Durable fix is to migrate `k8s/nfs/nfs-server.yaml` to a maintained NFS-server image (its own validated change + e2e), NOT to disable digest tracking. Trigger: the next time this recurs, or any maintained alternative that passes `TEST_NFS_HOST`-style verification.
