...
- [ ] **MetalLB is "tag-without-release" upstream-blocked, NOT stale.** `metallb/metallb` is held at v0.16.0 because `v0.16.1` is a tag with no release object (latest release is the `metallb-chart-*` Helm tag, which the `github-releases` datasource skips). Do NOT chase the tag or add a config workaround — wait for a real upstream release. Optional: switch the MetalLB renovate comment to `datasource=github-tags ... extractVersion=^v(?<version>.*)$` if tag-only patches should be tracked. (trivy's analogous v0.71.1 tag-only block lifted 2026-06-15 — upstream published the release; bumped to 0.71.1, now 0.71.2.)
- [ ] **cloud-provider-kind is pre-1.0** (0.11.1) and load-bearing for `make install-all` — watch its changelog for breaking changes on each minor bump. In particular, the `--gateway-channel disabled` flag in `scripts/kind-add-cloud-provider-kind.sh` is **load-bearing** (it stops CPK's embedded Gateway API CRD reconcile from touching the cluster's shared CRDs during the MetalLB→CPK migration); re-verify the flag still exists and behaves on every CPK bump (`docker run …:vX.Y.Z --help` — confirmed present at v0.11.1). Note v0.11.1 now vendors gateway-api **v1.5.1** (v0.10.0 vendored v1.4.0), so the original `< v1.5.0`-CRD `safe-upgrades` VAP-denial crash may not reproduce identically; the `gateway-test.yml` "Regression guard — CPK survives a restart …" step is the standing verification and fails if the flag is dropped.
