...
- [ ] **Re-verify all 6 Gateway API controllers together when GW API v1.6 ships.** Traefik, Istio, NGF, Contour, Envoy Gateway, and kgateway share the **experimental-channel** Gateway API v1.5.1 CRDs. Bumping the shared CRDs forces a joint re-verification — the oldest vendored API client gates the whole set (this is how HAProxy was dropped: it predated the v1.2.0 `supportedFeatures` `[]string`→`[]object` change). Trigger: `kubernetes-sigs/gateway-api` v1.6.0 release. Safety net: `gateway-test.yml` weekly cron.
