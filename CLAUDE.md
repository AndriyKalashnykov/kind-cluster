...
- **e2e-metallb.yml** — weekly cron (Sunday 04:00 UTC) + `workflow_dispatch` + push on its MetalLB/migration scripts and the shared smoke chain (`scripts/kind-add-metallb.sh`, `scripts/migrate-from-metallb.sh`, `scripts/e2e-migrate-smoke.sh`, `scripts/kind-add-cloud-provider-kind.sh`, `scripts/e2e-smoke.sh`, `scripts/lib.sh`, the workflow file) (positive-include `paths:` filter is intentional — runs when its own or the shared smoke-chain code changes; the shared scripts are included so a fix that only breaks the MetalLB path, which `ci.yml`'s cloud-provider-kind `e2e` job doesn't exercise, isn't missed until the weekly cron). Mirrors the `e2e` job but installs **MetalLB** instead of cloud-provider-kind; same K1.5 route-readiness poll before `LB=metallb make e2e-smoke`. Also installs cert-manager (with `ENABLE_GATEWAY_API=false` — this job's TLS path is **classic Ingress only**, so cert-manager needs no Gateway API support and the job installs no Gateway API machinery; the migration step's `cloud-provider-kind` is independently hardened against the Gateway API `safe-upgrades` VAP via `--gateway-channel disabled`, see the convention note below) + `make tls` and runs the smoke with `TEST_TLS=yes` so `e2e-smoke.sh`'s trusted-HTTPS **LB-IP assertion** terminates through the Traefik Service's **MetalLB-assigned** LoadBalancer IP — closing the gap where trusted HTTPS was previously verified only under cloud-provider-kind (paths filter includes `scripts/kind-add-cert-manager.sh`, `scripts/kind-add-tls.sh`, `k8s/tls/**`). Final step runs `scripts/e2e-migrate-smoke.sh` to exercise the **MetalLB → cloud-provider-kind migration** path so regressions in `migrate-from-metallb.sh` surface here.
...
- **gateway-test.yml** — weekly cron (Sunday 07:00 UTC) + `workflow_dispatch` + push on the gateway scripts/manifests + the shared smoke chain (`scripts/kind-add-gateway-*.sh`, `k8s/gateway/**`, `scripts/kind-add-traefik.sh`, `scripts/e2e-smoke.sh`, `scripts/lib.sh`, the workflow file). Brings up `make install-all`, then `make gateway-traefik` (Traefik's Gateway API provider) + `make gateway-istio` (Istio) + `make gateway-nginx` (NGINX Gateway Fabric) + `make gateway-contour` (Project Contour) + `make gateway-envoy` (Envoy Gateway) + `make gateway-kgateway` (kgateway) + `make gateway-kong` (Kong/KIC), then `make cert-manager` + `make tls` + `make tls-all` (local-CA TLS), then `TEST_GATEWAY_API=yes TEST_TLS=yes make e2e-smoke` to assert all controllers front the same demo apps **and** that trusted HTTPS (local CA + sslip.io, no `-k`) works on the static `*.localdev.me` path, the Traefik Gateway-API `*.gw.localdev.me` path (its `websecure` entrypoint terminates on container port 8443, sharing the one Traefik pod's `:443` with the classic Ingress via per-SNI cert resolution — `make tls` wires both), and each gateway's `*.<lb-ip>.sslip.io` path (paths filter includes `scripts/kind-add-cert-manager.sh`, `scripts/kind-add-tls*.sh`, `k8s/tls/**`). The shared Gateway API CRDs are the **experimental** channel (Contour requires `TLSRoute@v1alpha2`); HAProxy Ingress was evaluated but dropped (v0.16.1 vendored a pre-Gateway-API-v1.2.0 client whose `GatewayClass.status.supportedFeatures` was `[]string` and crash-loops on the v1.5.1 `[]object` form — Envoy Gateway/kgateway vendor v1.5.1 exactly, Kong/KIC v1.3.0, all ≥ the v1.2.0 floor — see `docs/gateway-api-ingress.md`). A final **regression-guard** step restarts cloud-provider-kind with the experimental CRDs + `safe-upgrades` VAP present and asserts it does not crash-loop — this is the only standing job that exercises CPK startup with the VAP present, so it fails if the `--gateway-channel disabled` hardening (see the cloud-provider-kind convention note) is ever dropped (paths filter therefore also includes `scripts/kind-add-cloud-provider-kind.sh`). Off the default install-all path — Gateway API is opt-in.
- **ingress-test.yml** — weekly cron (Sunday 08:00 UTC) + `workflow_dispatch` + push on the alternative-ingress scripts/manifests + the local-CA TLS chain + the shared smoke chain (`scripts/kind-add-ingress-haproxy.sh`, `scripts/kind-add-ingress-nginx.sh`, `k8s/demo-apps-ingress-{haproxy,nginx}.yaml`, `scripts/kind-add-cert-manager.sh`, `scripts/kind-add-tls.sh`, `scripts/kind-add-tls-all.sh`, `k8s/tls/**`, `scripts/e2e-smoke.sh`, `scripts/lib.sh`, the workflow file). Brings up `make install-all`, then `make ingress-haproxy` (HAProxy/haproxytech) + `make ingress-nginx` (NGINX Inc. F5 OSS) as alternative **classic** Ingress controllers, then `make cert-manager` + `make tls` + `make tls-all` (local-CA TLS — `tls-all`'s Gateway specs all skip here, only the classic-Ingress `*.sslip.io` certs are wired), then `TEST_INGRESS_ALT=yes TEST_TLS=yes make e2e-smoke` to assert each fronts the same demo apps on its own LB IP via a distinct `ingressClassName` **and** that trusted HTTPS (local CA + sslip.io, no `-k`) works on each. **cert-manager runs with `ENABLE_GATEWAY_API=false`** here (set in the workflow step's `env:`) because this job's TLS path is classic Ingress only — no Gateway resources — and cert-manager's controller crash-loops at startup when `enableGatewayAPI=true` but no Gateway API CRDs are present; this job installs none (and since cloud-provider-kind runs with `--gateway-channel disabled` it no longer side-installs them — see the cloud-provider-kind convention note). Mirrors `e2e-metallb.yml`. Classic Ingress controllers never watch `GatewayClass`, so they're immune to the GW API v1.5.1 `supportedFeatures` crash. Off the default install-all path — opt-in.
...
- **cloud-provider-kind is started with `--gateway-channel disabled`** (`scripts/kind-add-cloud-provider-kind.sh`). The lab serves the Gateway API via dedicated controllers (Traefik/Istio/NGF/Contour/Envoy/kgateway/Kong), never via CPK's own Gateway API support, and CPK assigns LoadBalancer IPs to Services independently of its gateway channel (the Service-LoadBalancer controller path runs regardless of the channel — source-verified; the `standard`/`experimental`/`disabled` flag is confirmed present through v0.11.1). By **default** (`--gateway-channel standard`) CPK force-installs its embedded Gateway API CRDs at startup; the bundled version tracks CPK's vendored `sigs.k8s.io/gateway-api` (v1.4.0 at v0.10.0; **v1.5.1 at v0.11.1**). At v0.10.0 the embedded CRDs were **v1.4.0 (< v1.5.0)**, so on a cluster that already has the experimental Gateway API v1.5.1 CRDs + their `safe-upgrades` ValidatingAdmissionPolicy (which denies installing any GW API CRD < v1.5.0) that install was **denied** and CPK aborted (`Failed to start cloud controller`) — never assigning a LoadBalancer IP. This bit the **MetalLB → CPK migration** whenever a gateway controller was already installed (the migration starts CPK *after* those CRDs + VAP exist). `disabled` skips CPK's GW API CRD reconcile entirely (verified by source + an A/B spike at v0.10.0: no-flag → crash + no IP; `disabled` → clean start + IP). It is retained regardless of CPK's bundled version because the lab owns GW API CRD installation separately (`scripts/kind-add-gateway-api-crds.sh`, experimental v1.6.0 channel) and nothing relies on CPK installing them; `gateway-test.yml`'s regression guard re-confirms CPK survives a restart with the experimental CRDs + VAP present on the current pin. NOTE: since v0.11.1 vendors gateway-api v1.5.1, the original version-mismatch crash may not reproduce identically — the regression guard is the standing check on each CPK bump.
...
- **`renovate.json` sets `platformAutomerge: false` ON PURPOSE — do NOT "fix" it to `true`.** `main` requires the `ci-pass` status check (Ruleset + classic protection, `strict: true`). With `platformAutomerge: true`, GitHub native auto-merge can complete the merge in the ~1s window *before* the CI check registers — the registration race — landing a RED bump on `main` (this exact repo hit it: PRs #92/#99 merged red with zero required checks, then #101 merged red via the race even after `ci-pass` was made required, 2026-06-13). `false` makes Renovate merge via its own later run after re-confirming green. A `/project-review` / `/renovate` agent may flag `false` as a "deviation from the base template" — it is the documented race-safe choice for a required-check repo, not a deviation (see the `/renovate` skill §"Automerge is only as safe as the REQUIRED status checks").
- **`CLOUD_PROVIDER_KIND_VERSION` is Renovate-tracked via `datasource=docker depName=registry.k8s.io/cloud-provider-kind/cloud-controller-manager` (the CONTAINER IMAGE), NOT `github-releases` — do NOT switch it back.** The value is consumed only as a `docker run …:v$(VERSION)` tag, and CPK's image reaches `registry.k8s.io` via a manual kpromo promotion PR to `kubernetes/k8s.io` that lags the GitHub release by hours-to-days. A `github-releases` datasource (the prior pin) proposed v0.11.0 the day it was tagged (2026-06-24) while the image was unpublished, breaking e2e at `docker run` (`manifest unknown`) before any LB IP — Renovate PR #152. Renovate proposes the next bump only once the matching image tag is promoted. (Same datasource-vs-sink rule as the `/renovate` skill's "track the CONSUMED SINK" section.)
- **Diagram renderer bumps need a manual PNG regen (runbook).** Renovate tracks the C4-PlantUML `!include` version in `docs/diagrams/*.puml` and `PLANTUML_VERSION` in the Makefile. A bump that changes the render **fails the `diagrams-check` gate**, so the PR can't auto-merge — a `packageRule` in `renovate.json` (matchDepNames `plantuml-stdlib/C4-PlantUML` + `plantuml/plantuml`) sets `automerge: false` and labels it `needs-diagram-regen`. To finish such a PR: check out the branch, run `make diagrams`, and commit the regenerated `docs/diagrams/out/*.png`, then let CI go green and merge.
- **`tls-flow.puml` and `gateway-coexistence.puml` are intentionally PlantUML, not inline Mermaid.** The `/architecture-diagrams` skill mandates Mermaid for non-C4 sequence/flow diagrams, but these two are kept as PlantUML PNGs on purpose — they share the modern-flat C4 skinparam theme with the C4 set for one coherent, `<img width>`-controllable diagram family, and they render via the same pinned `plantuml/plantuml` image + `diagrams-check` gate. This is a documented exception to the Mermaid-for-non-C4 rule.
- **Operator tunables are documented in a committed `.env.example`** (KIND_CLUSTER_NAME, LB, REGISTRY_PORT, NFS_SERVER/NFS_PATH, VM NAME/CPUS/MEMORY/DISK). The Makefile `-include .env` (gitignored local override) before its `?=` defaults, and a `check-env` gate (first prereq of `static-check`) fails if `.env.example` goes missing. This lab runs no long-lived service, so the tunables are deployment knobs only — there are no runtime host/port/credential values to externalize.
...
- [ ] **Re-verify all 7 Gateway API controllers together when GW API v1.7 ships.** Traefik, Istio, NGF, Contour, Envoy Gateway, kgateway, and Kong share the **experimental-channel** Gateway API v1.6.0 CRDs (bumped from v1.5.1 by Renovate #173, automerged on green `gateway-test.yml` — that CI run *is* the joint re-verification of all 7 on v1.6.0). Bumping the shared CRDs forces a joint re-verification — the oldest vendored API client gates the whole set (this is how HAProxy was dropped: it predated the v1.2.0 `supportedFeatures` `[]string`→`[]object` change). Trigger: `kubernetes-sigs/gateway-api` v1.7.0 release. Safety net: `gateway-test.yml` weekly cron.
...
- [ ] **cloud-provider-kind is pre-1.0 and load-bearing for `make install-all`** — watch its changelog for breaking changes on each minor bump. On every CPK bump, re-verify the load-bearing `--gateway-channel disabled` flag still exists and behaves (`docker run …:vX.Y.Z --help`); the `gateway-test.yml` "Regression guard — CPK survives a restart with the experimental CRDs + VAP present" step is the standing verification and fails if the flag is dropped. Full rationale (the embedded-CRD / `safe-upgrades` VAP crash analysis) lives in the `--gateway-channel disabled` convention note above — don't duplicate it here.
- [ ] **Watch `sharedvolume/nfs-server` (in-cluster NFS image) — low bus-factor.** It's a v0.x single-author fork of itsthenetwork (~4★, first build Aug 2025), pinned immutable `v0.1.0` and digest-tracked in `k8s/nfs/nfs-server.yaml`; the immutable pin means it keeps working even if upstream goes dark. If it's ever yanked or needs a CVE patch with no successor release, fall back to re-pinning `itsthenetwork/nfs-server-alpine` at its then-current resolved digest and closing future mutable-tag bumps (the pre-migration behavior). Context: the migration off the mutable `:12` tag landed 2026-06-24 (PR #157), closing the recurring orphaned-config-blob e2e breaks (#149/#154) where re-resolving the mutable tag's digest orphaned the old one into an unpullable blob.
- [ ] **`jq` (`.mise.toml`) bumps are MANUAL — Renovate can't track its `jq-` tag prefix.** jqlang/jq publishes GitHub tags as `jq-1.8.2` (not `v1.8.2`/`1.8.2`); Renovate's mise manager auto-applies `extractVersion=^v?(?<version>.+)` which leaves the `jq-`, so the extracted `jq-X.Y.Z` never compares to the bare pin — jq is *detected* in the dashboard but no bump is ever proposed. **Verified 2026-06-29 that neither fix works** for the aqua backend: a packageRule `extractVersion` does NOT override the mise manager's auto value, and `.mise.toml` `version_prefix = "jq-"` is NOT read by Renovate for aqua (it IS for the `github:` backend). So there is no config fix today. Bump jq by hand when `mise latest aqua:jqlang/jq` shows newer (last done: 1.8.1→1.8.2, 2026-06-29). Trigger to retire this item: Renovate adds aqua-prefix handling, or jq drops the `jq-` tag prefix. (Pattern captured in the `/renovate` skill.)
...
- **Versions** — every controller/chart/tool version cell against its source of truth: `.mise.toml` (kind, kubectl, trivy, act, gitleaks, hadolint, shellcheck, actionlint, jq, bats) and the `scripts/kind-add-*.sh` pins (`GATEWAY_API_VERSION` — the shared experimental CRD channel, `ISTIO_VERSION`, `NGF_VERSION`, `CONTOUR_VERSION`, `ENVOY_GATEWAY_VERSION`, `KGATEWAY_VERSION`, `KONG_VERSION`, `TRAEFIK_CHART_VERSION`, `HAPROXY_INGRESS_VERSION`, `NGINX_INGRESS_VERSION`, `CERT_MANAGER_VERSION`, `METALLB_VERSION`, `CSI_DRIVER_NFS_VERSION`, `METRICS_SERVER_CHART_VERSION`, `HEADLAMP_CHART_VERSION`) + `CLOUD_PROVIDER_KIND_VERSION` in the Makefile.
- **Diagram tech-strings** — the `docs/diagrams/c4-container.puml` and `c4-deployment.puml` sources embed the SAME version strings as prose (Gateway API CRD channel, Envoy Gateway, HAProxy chart, metrics-server, etc.). `diagrams-check` only catches puml-vs-PNG drift, NOT puml-vs-source-of-truth drift, so a version bump can leave the rendered architecture PNGs stale while the gate stays green. On every `/readme` (or `/project-review`) pass, grep the two `.puml` files for each controller/chart version and reconcile against the same pins above; if any differ, edit the puml, `make diagrams`, and commit the regenerated PNGs.
